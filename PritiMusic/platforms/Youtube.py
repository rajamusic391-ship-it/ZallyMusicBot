        ...
        async def audio_dl(vid_id):
            try:
                ...
                headers = {
                    ...
                    "X-Audio-Quality": "high",  # Request high quality audio
                    "X-Format-Preference": "opus,m4a,mp3",  # Preferred audio formats
                    "X-Bitrate-Preference": "256,192,128",  # Bitrate preferences
                    "X-Enhance-Audio": "true"  # Request enhanced audio processing
                }
                ...
                # First get audio info with enhanced quality parameters
                audio_params = {
                    "enhance_audio": "true",
                    "prefer_bitrate": "256",  # Prefer 256kbps for best quality
                    "optimize_for_voice_chat": "true",  # Optimize for Telegram voice chat
                    "include_metadata": "true",  # Include audio metadata
                    "normalize_audio": "true",  # Normalize audio levels
                    "preserve_bass": "true"  # Preserve bass frequencies
                }
                ...
                if status == 'success':
                    ...
                    if 'audio_qualities' in songData:
                        ...
                        best_quality = None
                        quality_priority = ['256kbps', '192kbps', '128kbps', 'high', 'medium']
                        
                        for quality in quality_priority:
                            if quality in qualities:
                                best_quality = qualities[quality]
                                break
                        
                        if best_quality and best_quality != audio_url:
                            audio_url = best_quality
                            logger.info(f"Selected enhanced audio quality: {quality}")
                    
                    # Add audio enhancement parameters to download URL
                    if '?' in audio_url:
                        audio_url += '&enhance=true&normalize=true&preserve_bass=true'
                    else:
                        audio_url += '?enhance=true&normalize=true&preserve_bass=true'
                    
                    result = await download_with_requests(audio_url, filepath, headers)
                    if result:
                        logger.info(f"Downloaded enhanced audio for {vid_id}")
                        return result
        ...
        async def song_audio_dl():
            try:
                ...
                headers = {
                    ...
                    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36",
                    "Accept": "application/json",
                    "X-Audio-Quality": "premium",  # Request premium quality
                    "X-Format-Preference": "opus,m4a,mp3",
                    "X-Bitrate-Preference": "320,256,192",  # Higher bitrate preference
                    "X-Enhance-Audio": "true",
                    "X-Audio-Enhancement": "bass_boost+normalize+cleaning"  # Audio enhancement options
                }
                ...
                # Enhanced audio parameters for song downloads
                audio_params = {
                    "enhance_audio": "true",
                    "prefer_bitrate": "320",  # Highest quality for songs
                    "optimize_for_voice_chat": "true",
                    "include_metadata": "true",
                    "normalize_audio": "true",
                    "preserve_bass": "true",
                    "enhance_clarity": "true",  # Enhance audio clarity
                    "dynamic_range": "wide",  # Wide dynamic range
                    "audio_profile": "telegram_optimized"  # Telegram-optimized profile
                }
                ...
                if status == 'success':
                    ...
                    # Check for enhanced audio options
                    if 'enhanced_audio_url' in audioData:
                        audio_url = audioData['enhanced_audio_url']
                        logger.info(f"Using enhanced audio URL for {title}")
                        ...
                        for quality_key, quality_url in audioData['audio_qualities'].items():
                            if '320' in quality_key or 'highest' in quality_key.lower():
                                ...
                                logger.info(f"Selected 320kbps quality for {title}")
                                break
                    
                    # Add enhancement parameters
                    if '?' in audio_url:
                        audio_url += '&enhance=true&normalize=true&bass_boost=true&cleaning=true'
                    else:
                        audio_url += '?enhance=true&normalize=true&bass_boost=true&cleaning=true'
                    
                    result = await download_with_requests(audio_url, filepath, headers)
                    if result:
                        logger.info(f"Downloaded crystal clear audio for: {title}")
                        return result
